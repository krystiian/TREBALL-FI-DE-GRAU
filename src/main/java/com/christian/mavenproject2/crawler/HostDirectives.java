...
public class HostDirectives {
	...
	/**
	 * Change the user agent string used to crawl after initialization. This
	 * will reorder (recreate) the list of user agent directives for this host.
	 *
	 * @param userAgent
	...
	public int checkAccess(String path) {
		...
		// When checking rules, the list of rules is already ordered based on
		// the
		// match of the user-agent of the clause with the user-agent of the
		// crawler.
		// The most specific match should come first.
		//
		// Only the most specific match is obeyed, unless ignoreUADiscrimination
		// is
		// enabled. In that case, any matching non-wildcard clause that
		// explicitly
		// disallows the path is obeyed. If no such rule exists and any UA in
		// the list
		...
		for (UserAgentDirectives ua : rules) {
			...
			// This is a wildcard rule that disallows access. The verdict is
			// stored,
			// but the other rules will also be checked to see if any specific
			// UA is allowed
			// access to this path. If so, that positive UA discrimination is
			// ignored
