...
public class PathRule {
	...
	/**
	 * Match a pattern defined in a robots.txt file to a path Following the
	 * pattern definition as stated on:
	 * https://support.google.com/webmasters/answer/6062596?hl=en&ref_topic=6061961
	 *
	 * This page defines the following items: * matches any sequence of
	 * characters, including / $ matches the end of the line
	 *
	 * @param pattern
	...
	public static Pattern robotsPatternToRegexp(String pattern) {
		...
		// If the pattern is empty, match only completely empty entries, e.g.,
		// none as
		...
		for (int pos = 0; pos < pattern.length(); ++pos) {
			...
			if (ch == '\\') {
				...
				if (quoteBuf.length() > 0) {
					// The quoted character buffer is not empty, so add them
					// before adding
				...
				// A $ at the end of the pattern indicates that the path should
				// end here in order
				// to match
