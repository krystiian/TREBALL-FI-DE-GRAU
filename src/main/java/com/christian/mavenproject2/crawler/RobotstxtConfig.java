...
public class RobotstxtConfig {
	...
	/**
	 * Whether to ignore positive user-agent discrimination. There are websties
	 * that use a white-list system where they explicitly allow Googlebot but
	 * disallow all other bots by a "User-agent: * Disallow: /" rule. Setting
	 * this setting to true will ignore the user-agent and apply the "Allow"
	 * rule to all user-agents. This can still be overridden when a robots.txt
