...
/**
 * The UserAgentDirectives class stores the configuration for a single user
 * agent as defined in the robots.txt. The user agent string used depends on the
 * most recent User-agent: definition in the robots.txt file.
 */
public class UserAgentDirectives {
	...
	static class PathComparator implements Comparator<PathRule> {
		...
		/**
		 * Compare two paths. If lhs matches and rhs does not, this will return
		 * -1 If rhs matches and lhs does not, this will return 1 If both match
		 * or both do not match,, this will return the result of a numeric
		 * comparison of the length of both patterns, where the longest (=most
		 * specific) one will come first.
		 */
		@Override
	...
	/**
	 * Match the current user agent directive set with the given user agent. The
	 * returned value will be the maximum match length of any user agent.
	 *
	 * @param userAgent
	 *            The user agent used by the crawler
	 * @return The maximum length of a matching user agent in this set of
	 *         directives
	 */
	...
	/**
	 * Add a rule to the list of rules for this user agent. Valid rules are:
	 * sitemap, crawl-delay, host, allow and disallow. These are based on the
