...
public class WebCrawler implements Runnable {
	 ...
	 * crawl except those with a nofollow flag.
	 *
	 * @param url
	...
	public boolean shouldVisit(Page referringPage, WebURL url) {
		if (myController.getConfig().isRespectNoFollow()) {
			return !((referringPage != null && referringPage.getContentType() != null
					...
					&& ((HtmlParseData) referringPage.getParseData()).getMetaTagValue("robots").contains("nofollow"))
	...
	/**
	 * Determine whether links found at the given URL should be added to the
	 * queue for crawling. By default this method returns true always, but
	 * classes that extend WebCrawler can override it in order to implement
	 * particular policies about which pages should be mined for outgoing links
	 * and which should not.
	 *
	 * If links from the URL are not being followed, then we are not operating
	 * as a web crawler and need not check robots.txt before fetching the single
	 * URL. (see definition at http://www.robotstxt.org/faq/what.html). Thus
	 * URLs that return false from this method will not be subject to robots.txt
	 * filtering.
	 *
	 * @param url
	 *            the URL of the page under consideration
	 * @return true if outgoing links from this page should be added to the
	 *         queue.
	 */
	...
	public Boolean isUnderCondition(mainMenu menu, String href) {
